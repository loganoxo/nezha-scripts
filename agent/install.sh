...
install() {
    ...
    if [ -z "$LOGAN_NZ_SERVER" ]; then
        err "LOGAN_NZ_SERVER should not be empty"
        exit 1
    fi

    if [ -z "$LOGAN_NZ_CLIENT_SECRET" ]; then
        err "LOGAN_NZ_CLIENT_SECRET should not be empty"
        exit 1
    fi
    ...
    env="$(env | grep ^LOGAN_NZ_ | sed "s|^LOGAN_||" | xargs)"
